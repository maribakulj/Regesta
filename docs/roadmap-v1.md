...
| WP-6 conformance | ◐ — **mechanism + first profile ✅** (`regesta.conformance`: institutional profiles as diagnostic check sets over the WEMI projection, policy-gated by the acceptance threshold; the **Linked Art / Louvre** profile encodes the public LA object model; `regesta conformance --profile linked-art`). Dataless slice done; **institutional *certification*** (a passing report on real BnF/Louvre samples against their private acceptance criteria — DoD #5) stays partnership-gated (§ 7). The strict official-schema LA validation remains a separate test-only eval. |
| WP-7 scale · WP-9 release | ✗ |
