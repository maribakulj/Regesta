...
| WP-4 spokes | ◐ — **4 importers in ✅**: INTERMARC-SRU, MARC21 (MARCXML), Dublin Core, **MODS (LoC, nested focused parser, 3 real fixtures)**; **2 round-trips ✅** (DC + MARC21↔floor, loss measured, id-stable & idempotent); shared `marcxml` core; multi-spoke convergence capstone; canonical→WEMI floor ✅; N-Triples + additive/pure CRM out ✅; **MODS/DC export beyond DC+MARC21, IIIF, Linked Art, Turtle/JSON-LD not built; MARC21↔LRMoo is at the *floor* level, not conformance-checked** |
